...
class BaseSGDRegressor(RegressorMixin, BaseSGD):
    ...
    def partial_fit(self, X, y, sample_weight=None):
        ...
        if hasattr(X, "batch_mode") and X.batch_mode:
            return self._partial_fit_batch_mode(X, sample_weight)
    ...
    def _partial_fit_batch_mode(self, data_bunch, sample_weight=None):
        """
        NEW METHOD: partial_fit with batch mode from load_postgres.

        Parameters
        ----------
        data_bunch : Bunch
            Bunch object with batch_mode=True from load_postgres
        """
        print(f"\n{'='*60}")
        print(f"PARTIAL_FIT with BATCH MODE")
        print(f"{'='*60}")
        print(f"Total samples: {data_bunch.n_samples}")
        print(f"Features: {data_bunch.n_features}")

        batch_count = 0

        for X_batch, y_batch in data_bunch.data:
            batch_count += 1

            # Call original partial_fit for this batch
            self._partial_fit(
                X_batch,
                y_batch,
                alpha=self.alpha,
        ...
        print(f"Batch partial_fit complete. Total batches: {batch_count}")
        print(f"{'='*60}\n")
        return self
    ...
    def fit(self, X, y, coef_init=None, intercept_init=None, sample_weight=None):
        ...
        self._more_validate_params()
        if hasattr(X, "batch_mode") and X.batch_mode:
            return self._fit_batch_mode(X, coef_init, intercept_init, sample_weight)
    ...
    def _fit_batch_mode(
        self, data_bunch, coef_init=None, intercept_init=None, sample_weight=None
    ):
        """
        NEW METHOD: Fit using batch mode from load_postgres.
        Uses partial_fit internally for each batch.
        ...
        """
        print(f"\n{'='*60}")
        print(f"FIT with BATCH MODE (internal partial_fit)")
        print(f"{'='*60}")
        print(f"Total samples: {data_bunch.n_samples}")
        print(f"Features: {data_bunch.n_features}")
        ...
        for X_batch, y_batch in data_bunch.data:
            ...
            # Use partial_fit for this batch
            self.partial_fit(X_batch, y_batch, sample_weight=sample_weight)

            if batch_count % 10 == 0:
                print(f"  Processed {batch_count} batches...")

        print(f"Batch fitting complete. Total batches: {batch_count}")
        print(f"{'='*60}\n")
        return self
