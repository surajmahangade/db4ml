...
def _postgres_batch_generator(query, db_config, batch_size, target_column):
    conn = psycopg2.connect(**db_config)
    cursor = conn.cursor(name="sklearn_batch_cursor")
    ...
    try:
        ...
        if not first_rows:
            return  # no data
        column_names = [desc[0] for desc in cursor.description]  # move here

        # Yield first batch
        data = np.array(first_rows, dtype=np.float64)
        ...
        # Continue yielding remaining batches
        while True:
...
# Add main loading function
def load_postgres(
    query, db_config, *, batch_size=1000, target_column=None, return_X_y=False
):
    """
    Load data from PostgreSQL database with automatic batch detection.
    ...
    target_column : int, str, or None, default=None
        Target column identifier

    return_X_y : bool, default=False
        If True, returns (data, target) tuple

    Returns
    -------
    data : Bunch
        Dictionary-like object with:
        - data : ndarray or generator
        - target : ndarray or None
        - feature_names : list
        - DESCR : str
        - batch_mode : bool (True if using batches)
        - n_samples : int
        - n_features : int
    """
    if not HAS_PSYCOPG2:
    ...
    # Get column names
    cursor.execute(f"SELECT * FROM ({query}) as subq LIMIT 0")
    ...
    if target_column is not None:
        if isinstance(target_column, str):
            ...
            target_idx = target_column
        feature_names = [col for i, col in enumerate(column_names) if i != target_idx]
    else:
        feature_names = column_names

    # Description
    descr = f"PostgreSQL Dataset\n{'='*50}\n"
    descr += f"Samples: {n_samples}\nFeatures: {len(feature_names)}\n"

    from sklearn.utils import Bunch

    # Decide: batch mode or full load
    if batch_size < n_samples:
        # BATCH MODE
        generator = _postgres_batch_generator(
            query, db_config, batch_size, target_column
        )
